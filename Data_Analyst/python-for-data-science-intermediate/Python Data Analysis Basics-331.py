...
result = template.format(name=artist, year=birth_year)
print(result)
...
def artist_summary(name):
    ...
    template = "There are {num} artworks by {n} in the data set"
    final_string = template.format(num=num_artworks, n=name)
    return final_string
...
for row in pop_millions:
    string = template.format(name=row[0], pop=row[1])
    print(string)
...
for name,frq in gender_freq.items():
    template = "There are {fr:,} artworks by {n} artists"
    print(template.format(fr=frq, n=name))